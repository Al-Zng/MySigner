        ...
        NavigationView {
            ...
            .sheet(item: $pickingType) { type in
                DocumentPicker(type: type) { url in
                    ...
                    case .ipa:
                        ipaURL = url
                        // Auto-fill name from IPA filename
                        if appName.isEmpty {
...
// MARK: - Signing Engine (Real Logic)
struct SigningEngine {
    static func sign(
        ...
        // 6. Re-sign with codesign (requires valid cert in keychain on macOS)
        // On iOS device this step is handled by the system after sideloading
        // For a real device signing flow, zsign binary would be called here
...
struct DownloadsView: View {
    ...
    func startDownload() {
        ...
        var item = DownloadItem(name: name, size: "جاري التحميل...", url: url, progress: 0.0)
                ...
                ZStack {
                    ...
                    VStack(spacing: 16) {
                        ...
                        Button("Add") {
                            ...
                            showAdd = false
                            certName = ""; p12URL = nil; provisionURL = nil
                        }
